#!/usr/bin/env python3
"""
Quick Memory Monitor - Non-interactive version for VS Code integration

Usage:
    python3 quick_monitor.py [duration_hours] [interval_seconds]
    
Examples:
    python3 quick_monitor.py 1 60        # 1 hour, 60 second intervals
    python3 quick_monitor.py 3 30        # 3 hours, 30 second intervals
    python3 quick_monitor.py             # Default: 1 hour, 60 seconds
"""
...
class QuickMemoryMonitor:
    def __init__(self, duration_hours=1, sample_interval=60):
        self.process = None
    ...
    def monitor(self):
        ...
        print(f"🔍 Quick Memory Monitor Started")
        print(f"📊 Duration: {self.duration_hours} hours")
        print(f"⏱️  Sample interval: {self.sample_interval} seconds")
        print(f"⏰ Will complete at: {datetime.fromtimestamp(time.time() + duration_seconds).strftime('%H:%M:%S')}")
    ...
    def generate_report(self):
        ...
        print("\n" + "="*60)
        print("📊 QUICK MEMORY MONITORING REPORT")
        print("="*60)
        ...
        print("\n📋 RECOMMENDATIONS:")
        if growth_rate > 5:
            print("• Check memory_leak_debug.log for detailed logging")
            ...
            print("• Continue monitoring for longer periods if needed")
        
        # Save detailed report
        report_file = f"quick_memory_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        try:
            with open(report_file, 'w') as f:
                f.write("QUICK MEMORY MONITORING REPORT\n")
                f.write("="*50 + "\n\n")
                f.write(f"Duration: {total_time:.2f} hours\n")
                ...
                f.write("SAMPLE DATA:\n")
                f.write("Time\t\tMemory(MB)\tIncrease(MB)\n")
                f.write("-" * 40 + "\n")
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Invalid duration, using default 1 hour")
    ...
    print(f"🔍 Quick Memory Monitor")
    print(f"📊 Configuration: {duration} hours, {interval} second intervals")
    print("=" * 50)
    
    monitor = QuickMemoryMonitor(duration, interval)
