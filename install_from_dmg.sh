...
check_service_status() {
    ...
    # Check if service is loaded in launchctl (using the Label from plist, not filename)
    if launchctl list | grep -q "$service_label"; then
        echo -e "${GREEN}✅ $service_name is loaded in launchctl${NC}"

        # Check if process is actually running
        if pgrep -f "$service_name" > /dev/null; then
            echo -e "${GREEN}✅ $service_name process is running${NC}"

            # Get process count to verify no multiple instances (use exact match to avoid cross-matching)
            local process_count
            if [ "$service_name" = "ClipboardMonitor" ]; then
                # For main service, exclude MenuBar processes
                process_count=$(ps aux | grep "ClipboardMonitor" | grep -v "ClipboardMonitorMenuBar" | grep -v grep | wc -l | tr -d ' ')
            else
                # For MenuBar, use exact match
                process_count=$(pgrep -f "$service_name" | wc -l | tr -d ' ')
            fi

            if [ "$process_count" -eq 1 ]; then
                echo -e "${GREEN}✅ Single instance confirmed (no multiple spawning)${NC}"
            else
                echo -e "${YELLOW}⚠️  Multiple instances detected: $process_count processes${NC}"
            fi
            return 0
        else
            echo -e "${YELLOW}⚠️  $service_name is loaded but process not running${NC}"
            show_service_logs "$log_prefix" "$service_name"
            return 1
        fi
    else
        echo -e "${RED}❌ $service_name is not loaded in launchctl${NC}"

        # Check if process is running anyway (manual start)
        if pgrep -f "$service_name" > /dev/null; then
            echo -e "${YELLOW}⚠️  Process is running but not via LaunchAgent${NC}"
            local process_count
            if [ "$service_name" = "ClipboardMonitor" ]; then
                # For main service, exclude MenuBar processes
                process_count=$(ps aux | grep "ClipboardMonitor" | grep -v "ClipboardMonitorMenuBar" | grep -v grep | wc -l | tr -d ' ')
            ...
            echo -e "${BLUE}   Process count: $process_count${NC}"
            return 0
        else
            show_service_logs "$log_prefix" "$service_name"
            return 1
...
show_service_logs() {
    ...
    if [[ -f "$log_dir/${log_prefix}.out.log" ]] && [[ -s "$log_dir/${log_prefix}.out.log" ]]; then
        ...
        # Check if output log contains error indicators
        if grep -qi "error\|exception\|traceback\|failed\|cannot\|unable" "$log_dir/${log_prefix}.out.log"; then
            has_errors=true
        fi
    fi
...
check_log_files() {
    ...
    # Check main service output log for errors
    if [[ -f "$log_dir/ClipboardMonitor.out.log" ]]; then
        if [[ -s "$log_dir/ClipboardMonitor.out.log" ]]; then
            ...
            if grep -qi "error\|exception\|traceback\|failed\|cannot\|unable" "$log_dir/ClipboardMonitor.out.log"; then
                echo -e "${YELLOW}⚠️  Issues found in ClipboardMonitor.out.log (last 10 lines):${NC}"
    ...
    # Check menu bar service output log for errors
    if [[ -f "$log_dir/ClipboardMonitorMenuBar.out.log" ]]; then
        if [[ -s "$log_dir/ClipboardMonitorMenuBar.out.log" ]]; then
            ...
            if grep -qi "error\|exception\|traceback\|failed\|cannot\|unable" "$log_dir/ClipboardMonitorMenuBar.out.log"; then
                echo -e "${YELLOW}⚠️  Issues found in ClipboardMonitorMenuBar.out.log (last 10 lines):${NC}"
...
main() {
    ...
    check_service_status "ClipboardMonitor" "com.clipboardmonitor" "ClipboardMonitor"
    main_service_status=$?

    check_service_status "ClipboardMonitorMenuBar" "com.clipboardmonitor.menubar" "ClipboardMonitorMenuBar"
    menubar_service_status=$?

    echo ""
    print_step "📋 Final system verification..."

    # Additional verification: Check actual running processes
    echo -e "${BLUE}🔍 Verifying running processes...${NC}"
    main_processes=$(ps aux | grep "ClipboardMonitor" | grep -v "ClipboardMonitorMenuBar" | grep -v grep | wc -l | tr -d ' ')
    menubar_processes=$(ps aux | grep "ClipboardMonitorMenuBar" | grep -v grep | wc -l | tr -d ' ')
    total_processes=$((main_processes + menubar_processes))

    if [ "$total_processes" -gt 0 ]; then
        echo -e "${GREEN}✅ Found $total_processes ClipboardMonitor process(es) running${NC}"
        echo -e "${BLUE}   • Main service: $main_processes process(es)${NC}"
        echo -e "${BLUE}   • Menu bar app: $menubar_processes process(es)${NC}"

        if [ "$main_processes" -eq 1 ] && [ "$menubar_processes" -eq 1 ]; then
            echo -e "${GREEN}✅ Perfect! Exactly 1 of each process type${NC}"
        elif [ "$total_processes" -eq 2 ]; then
            echo -e "${GREEN}✅ Good! Total of 2 processes running${NC}"
        elif [ "$total_processes" -eq 1 ]; then
            echo -e "${YELLOW}⚠️  Only 1 process running (expected 2)${NC}"
        else
            echo -e "${YELLOW}⚠️  $total_processes processes running (expected 2)${NC}"
        fi
    else
        echo -e "${RED}❌ No ClipboardMonitor processes found${NC}"
    fi
